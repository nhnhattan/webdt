	...
	*/
	class product
	{
		...
		public function insert_product($date,$files){
			...
			$productCode = mysqli_real_escape_string($this->db->link, $date['product_code']);
			...
			if($productCode =="" || $productName == "" || $productQuantity == "" || $category == "" || $brand == "" || $product_desc == "" || $price == "" || $type == "" || $file_name == ""){
				$alert = "<span class='error'>Fiedls must be not empty</span>";
				return $alert;
			}else{
				move_uploaded_file($file_temp, $uploaded_image);

				$query = "INSERT INTO product(productName,product_code,product_remain,productQuantity,catId,brandId,product_desc,price,type,image) VALUES('$productName','$productCode','$productQuantity','$productQuantity','$category','$brand','$product_desc','$price','$type','$unique_image') ";
		...
		public function show_slider(){
			$query = "SELECT * FROM slider where type='0' order by sliderId desc";
		...
		public function show_product_warehouse(){
			...
			"SELECT product.*, warehouse.*
			 ...
			 FROM product INNER JOIN warehouse ON product.productId = warehouse.id_sanpham
								
			 order by warehouse.sl_ngaynhap desc ";
		...
		public function search_product()
		{
			...
			"SELECT product.*, category.catName, brand.brandName
			 ...
			 FROM product INNER JOIN category ON product.catId = category.catId
								INNER JOIN brand ON product.brandId = brand.brandId
			 WHERE productName like '%".$term."%' or product_code like '%".$term."%' or productName like '%".$term."%'
			 order by product.productId desc
			 ";
		...
		public function update_product($data,$files,$id){
			...
			if($product_code == "" || $productName=="" || $productQuantity=="" || $brand=="" || $category=="" || $product_desc=="" || $price=="" || $type==""){
				...
				$result = $this->db->update($query);
					if($result){
						echo "<script>alert('Sản phẩm Updated thành công')</script><script> window.location = 'productlist.php' </script>";
					}else{
		...
		{
			$query = "SELECT * FROM product order by productId asc LIMIT 12 ";
			$result = $this->db->select($query);
			return $result;
		}
		public function getproduct_secondnormal()
		{
			$query = "SELECT * FROM product order by productId desc LIMIT 12 ";
			$result = $this->db->select($query);
			return $result;
		}

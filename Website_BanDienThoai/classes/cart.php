	...
	class cart
	{
		...
		public function add_to_cart($id, $quantity)
		{
			...
			if($result['product_remain']>$quantity){
				...
				$query_insert = "INSERT INTO cart(productId,productName,quantity,sId,price,image) VALUES('$id','$productName','$quantity','$sId','$price','$image' ) ";
		...
		public function update_quantity_Cart($proId,$cartId, $quantity)
		{
			...
			$query_product = "SELECT * FROM product WHERE productId = '$proId' ";
		...
		public function insertOrder($customer_id)
		{
			...
			if($get_product){
				while($result = $get_product->fetch_assoc()){
					...
					$query_order = "INSERT INTO `order`(productId,productName,quantity,price,image,customer_id) VALUES('$productid','$productName','$quantity','$price','$image','$customer_id')";
		...
		{
			$query = "SELECT price FROM `order` WHERE customer_id = '$customer_id' ";
		...
		public function get_inbox_cart($id)
		{
			$query = "SELECT * FROM `order` where customer_id = '$id' ";
			$get_inbox_cart = $this->db->select($query);
			return $get_inbox_cart;
		}
		public function get_inbox_cart_list()
		{
		...
		public function get_orderdetail_byId($id)
		{
			$id = mysqli_real_escape_string($this->db->link, $id);
			$query = "SELECT `order`.*,customer.email,customer.name FROM `order`,customer WHERE `order`.id ='$id' and customer.id = `order`.`customer_id`";
